...
public class EstoqueModel {
        ...
        try (Connection conn = DBconnection.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    produtos.add(new Produto(id, nome, precoCusto, precoVenda, quantidade, tipo, oculto));
                }
            }
    ...
    public List<Produto> pesquisarProdutos(String termo, String tipoPesquisa, boolean apenasVisiveis, int tabelaAtiva) {
        List<Produto> produtos = new ArrayList<>();
        try (Connection conn = DBconnection.getConnection()) {
            String sql;
    ...
    public List<Produto> carregarProdutosVisiveis() {
        List<Produto> produtos = new ArrayList<>();
        try (Connection conn = DBconnection.getConnection()) {
            String sql = "SELECT p.pdt_id, p.pdt_nome, p.pdt_preco_custo, p.pdt_preco_venda, p.pdt_tipo, e.est_quantidade, e.oculto " +
                    "FROM produtos p " +
    ...
    public List<Produto> carregarProdutosOcultos() {
        List<Produto> produtos = new ArrayList<>();
        try (Connection conn = DBconnection.getConnection()) {
            String sql = "SELECT p.pdt_id, p.pdt_nome, p.pdt_preco_custo, p.pdt_preco_venda, p.pdt_tipo, e.est_quantidade, e.oculto " +
                    "FROM produtos p " +
        ...
        try (Connection conn = DBconnection.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                if (rs.next()) {
                    ...
                    return new Produto(id, nome, precoCusto, precoVenda, quantidade, tipo, oculto);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
